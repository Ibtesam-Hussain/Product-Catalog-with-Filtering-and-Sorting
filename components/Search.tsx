    ...
    <div className="w-full">
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {filteredProducts.map((product: Product) => (
          <div key={product.id} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow p-4 flex flex-col">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-2">{product.short_description}</p>
            <span className="text-blue-600 dark:text-blue-400 font-semibold text-lg">${product.price}</span>
          </div>
        ))}
        {/* {filteredProducts.length === 0 && (
          <div className="col-span-full text-center text-gray-400 dark:text-gray-500 py-4">No products found for "{searchTerm}"</div>
        )} */}
      </div>
    </div>
  );
